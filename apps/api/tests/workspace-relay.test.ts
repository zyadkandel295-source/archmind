...
describe("same-origin chat and file relay", () => {
  ...
  it("starts Google auth without a bearer token by navigating to the trusted API", async () => {
    vi.stubEnv("API_INTERNAL_URL", "https://api.example");
    const fetcher = vi.fn();
    vi.stubGlobal("fetch", fetcher);
    ...
    expect(response.headers.get("Location")).toBe("https://api.example/api/auth/google?state=%2Fprofile");
    expect(fetcher).not.toHaveBeenCalled();
  });
});
