...
export async function runAssistantChat(input: { env: Env; store: MemoryStore; rag: RagService; assistantId: string; userId: string; message: string; sessionId?: string; conversationId?: string; responseLength: string; language: string; webSearch?: boolean; allowPublicAssistant?: boolean }) {
  ...
  const chunks = await input.rag.retrieve(assistant.id, input.message, input.userId);
  ...
  const answer = await input.rag.generateAnswer({ assistant, question: input.message, chunks, responseLength: input.responseLength, language: input.language, webSearch: input.webSearch, chatHistory });
