...
4. `FILE_GENERATION_MODEL` optionally overrides the existing model. The retired `nvidia/nemotron-3-ultra:free` selection is routed to the JSON-capable `nex-agi/nex-n2.5-mini:free` model, because the old identifier can return empty structured completions. The service keeps planning requests within the deployed provider allowance (6,000 tokens) while generating document units in smaller calls. Explicit current model IDs are preserved. Provider quotas and availability still apply.
