...
4. File generation is deliberately restricted to OpenRouter models in the free tier. The retired `nvidia/nemotron-3-ultra:free` selection, and any configured model without the `:free` suffix, are routed to the JSON-capable `nex-agi/nex-n2.5-mini:free` model. The service keeps planning requests within the deployed provider allowance (6,000 tokens) while generating document units in smaller calls. No paid model fallback is allowed; if the free provider is unavailable, the job fails with a retryable error.
